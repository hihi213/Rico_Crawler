...
> 현재 구현은 CSV 저장 + Playwright 기반이며, Docker/SQLite/Selectolax는 추후 단계로 계획합니다.

| 구분 | 선택 기술 | 선정 이유 |
| --- | --- | --- |
| Language | Python 3.11.14 | 풍부한 크롤링 라이브러리 생태계 및 빠른 프로토타이핑 지원. (단, Type Hinting을 엄격히 적용하여 안정성 확보) |
| Browser Control | Playwright | Selenium 대비 빠른 실행 속도, 안정적인 Auto-wait 기능, 브라우저 바이너리 관리의 용이성을 위해 선택. |
| Parsing | Selectolax (Planned) | 대량의 DOM 탐색 시 BeautifulSoup 대비 압도적인 파싱 속도(Low-level C binding) 제공. |
| Validation | Pydantic | 수집 데이터의 무결성 보장을 위해 Java의 Entity/DTO 검증 로직과 유사한 런타임 타입 검증 구현. |
| Persistence | SQLite (Planned) | 별도의 DB 설치 없이 로컬 파일 기반으로 상태(State)를 저장하고, 중단 시 이어하기(Resumability) 기능을 구현하기 위함. |
...
2. Robustness (견고함):
* Resumability: 크롤링 상태(마지막 페이지, 날짜)를 체크포인트로 기록하며, SQLite 저장은 추후 도입 예정.
* Data Integrity: Pydantic Validator를 통해 금액(`String` -> `Int`), 날짜 포맷 등을 정규화하여 저장.


3. Reproducibility (재현성):
* Docker는 도입하지 않았다. 과제 목적상 로컬 환경에서의 재현 가능성을 최우선으로 두고, README 기준 실행 절차를 표준화했다.
...
* 저장소 계층 분리로 SQLite → RDBMS 전환 시 영향 범위를 최소화합니다. (SQLite 도입은 추후 단계)
...
## 2026-02-09
### 실행 경로 단순화 (최종)
- 결정: 설치는 `install.py`, 실행은 `python main.py` 중심으로 통합
- 이유: 자동화 스크립트보다 CLI 옵션 최소화가 사용성에 더 유리
- 트레이드오프: `main.py` 옵션 문서화가 중요해져 README에 옵션을 명확히 기재

### 결정
## 2026-02-09: 목록 스키마 보강 및 정규화 확장

### 결정
- 목록 응답의 `bidPbancPgstCdNm`(공고게시 코드명)을 스키마에 추가한다.
- 일시 파서에 구분자 없는 포맷(`YYYYMMDDHHmmss`, `YYYYMMDDHHmm`)을 허용한다.
- Y/N 플래그 파서에 `True/False`, `1/0`도 허용한다.

### 이유(실무 관점)
- 코드명은 필터/보고서 가독성에 직접 영향을 주는 유의미 필드다.
- 누리장터 목록에서 구분자 없는 일시가 혼재되어 수집 누락이 발생했다.
- CSV 재로드/검증 시 불리언 재해석 실패를 방지해 재현성을 높인다.
...
## 2026-02-09: 필터 옵션 CLI 노출 및 텍스트 정규화 확대

### 결정
- CLI에서 공고종류/공고상태/진행상태 필터를 옵션으로 받을 수 있게 한다.
- HTML 엔티티 디코딩을 공지/첨부/개찰 포함 전체 텍스트 필드로 확대한다.
- 첨부 메타의 `bsne_clsf_cd`는 누락 가능성을 고려해 선택 필드로 완화한다.

### 이유(실무 관점)
- 기본은 전체 수집으로 두되, 옵션 필터로 범위를 제한할 수 있어야 운영 유연성이 높다.
...
## 2026-02-09: 표시용 CSV(view) 분리

### 결정
- 원본 데이터는 `data/`에 전체 컬럼(코드+코드명)으로 저장한다.
- 표시용 CSV는 `data/view/`에 분리 저장하며, `list.csv`와 `opening_result.csv`는 화면 기준 컬럼만 남긴다.
- 기존 데이터 기준으로 `data/view/`를 재생성하는 스크립트를 제공한다(`python scripts/make_view.py`).

### 이유(실무 관점)
- 코드/코드명 동시 노출로 가독성이 떨어져 화면 기준 뷰가 필요했다.
- 표시용과 원본을 분리하면 품질/정합성을 유지하면서 사용자 관점을 개선할 수 있다.
...
## 원본 JSON 스냅샷 옵션 도입 (2026-02-09)

### 결정
- API 응답 원본 JSON을 옵션으로 저장(기본 OFF)
- 전체 저장이 아닌 "예상 외(새) 필드가 감지된 응답만 선택 저장"으로 제한
- 스키마 미확정 항목 검증 및 재현성 확보용

### 이유(실무 관점)
- 샘플 확보 실패 시에도 후속 분석이 가능
- 이슈 재현 및 스키마 변경 추적에 유리
- 크롬에서 확인한 상세 응답이 빈 필드로 내려오는 경우가 있어 스키마를 확정하기 어려웠음
- 빈 필드로 인한 불확실성을 "변경 감지 스냅샷"으로 대응하기로 결정

### 진행 과정
1) config에 스냅샷 저장 옵션 추가 완료
2) 저장 위치/용량 영향은 README에 안내 완료
