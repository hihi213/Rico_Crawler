...
## 2026-02-09
### 실행/설정 자동화 파일명 단순화
- 결정: `install`/`run`/`set`(확장자 제거)로 실행 경로를 단축하고 역할별로 분리
- 이유: 면접관/사용자 입장에서 `python run`이 `python run.py`보다 짧고 기억하기 쉬움
- 트레이드오프: 확장자 제거로 파일 목적이 덜 명확할 수 있어 README에서 역할을 명시
