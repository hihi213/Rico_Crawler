...
## 개찰 점수 문자열+숫자 병행 저장 (2026-02-09)

### 결정
- `BidOpeningResult`에 문자열 점수와 숫자 점수를 병행 저장

### 이유(실무 관점)
- API가 숫자(0) 또는 문자열로 혼합 전달하는 케이스가 있어 타입 정합성 확보가 필요
- 문자열은 원본 표기, 숫자 필드는 통계/정렬에 활용 가능

### 진행 과정
1) 점수 필드를 문자열로 정규화
2) 숫자 필드(`*_num`) 추가 및 float 정규화
...
## 날짜 범위 모드 분리 및 유효성 검증 (2026-02-09)

### 결정
- 기본은 고정 날짜(재현성), 옵션으로 `search_range_days`를 추가해 동적 날짜 계산 지원
- 목록 API 페이로드에서 날짜 형식(YYYYMMDD)과 범위(start<=end) 검증 추가

### 이유(실무 관점)
- 과제 기준 재현성(동일 입력/동일 결과)과 운영성(최근 데이터 수집)을 모두 만족
- 잘못된 날짜 입력으로 API 실패/공백 결과가 발생하는 것을 사전에 차단

### 진행 과정
1) `config.yaml`에 `search_range_days` 옵션 추가
2) Service에서 페이로드 구성 시 동적 날짜 계산 또는 유효성 검증
