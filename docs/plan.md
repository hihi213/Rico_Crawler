...
- 스키마 보완 항목 (schema.md 기반, 우선순위 반영)
    - 필수
        - 상세 스키마 확정: `BidNoticeDetail` 필드 리스트 1차 확정 + JSON 필드명/도메인 매핑 표 작성
        - 날짜/금액/플래그 정규화 규칙 확장: 실제 포맷별 파서 규칙 명시
        - 응답 envelope 모델 추가: `ErrorCode`, `ErrorMsg` 포함 공통 응답 모델 정의
        - 고유키/관계 정의: 공고-첨부-개찰결과 FK/PK 관계 텍스트 다이어그램 추가
        - 테스트 기준 추가: 스키마 샘플 최소 건수 + 필수/선택 필드 검증 케이스 정의
    - 권장
        - 목록/상세 공통 키 정리: `bidPbancMap`/`pbancOrgMap`/목록행 공통 필드 모델 분리
        - 코드/코드명 사전: `CommCd` 코드 테이블 스키마 정의
        - 미확정 항목 트래킹: `2.1.2 추가 수집 필요 항목`을 상태/샘플/결론 표로 관리
        - 개찰결과 모델 추가: `oobsRsltList` + 요약(`pbancMap`) 엔티티 정의 및 연결키 명시
    - 보류(범위 외/후순위)
