...
- README에 "가정+변경 감지" 플로우 명시(예상 외 필드 감지 시 스냅샷 저장)
  - 배경: 크롬에서 확인한 상세 응답이 빈 필드로 내려와 스키마 확정이 어려웠음
- Future Works: 병렬화, 알림, 모니터링, 성능 개선
  ...
  - 현재 models.py은 단일 스레드 기반으로 파싱이 진행되지만, 대량 데이터 처리 시 비동기(asyncio) 처리를 도입하면 성능을 더 높일 수 있음.
  - (옵션) 원본 JSON 스냅샷 저장 기능
    - 기본값 OFF, 필요 시 ON으로 수집 응답 저장
    - 전체 저장 대신 "미확정 항목 포함 응답만 선택 저장"으로 제한
    - README에 용도/용량 영향/사용법 설명
  - 첨부파일 “메타데이터만 CSV에 저장”하는 구조, 실제 파일 다운로드는 추후개발
## 결정 보류/추후 반영(상세 메모)
