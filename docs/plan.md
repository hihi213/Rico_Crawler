...
- [ ] 목록 페이지 → 상세 페이지 진입 코드
  ...
  - [ ] Repository : 저장소는 파서 출력 확정 후 구현해야 CSV 컬럼/정규화가 안정적
  - [ ] Service : 서비스는 구성 요소가 준비된 뒤 연결해야 흐름 검증이 쉬움
- [ ] HTML/API 응답 가져와서 핵심 필드 추출
- [ ] 최소 1페이지(몇 개 공고) CSV에 저장까지 확인: Pydantic 모델/파서가 실제 응답과 잘 맞는지 검증
        

## 4단계: 안정성 기능 추가 (Day 3)

- [ ] tenacity 재시도: 가장 빈번한 실패(네트워크/타임아웃)를 먼저 줄이기
    - [ ] 네트워크/타임아웃에 대해 3회 retry
- [ ] 체크포인트 저장/재시작 : 중단 복구의 핵심이므로 MVP 검증 후 바로 추가
    - [ ] 마지막 페이지/공고 ID를 파일에 저장하고 재시작 시 사용
- [ ] 중복 방지 : 저장소/체크포인트 설계가 확정된 뒤 적용하는 게 안전
...
- [ ] CLI 모드 확장 : 실행 모드가 확정되어야  로그에 필요한 이벤트 범위가 결정됨
